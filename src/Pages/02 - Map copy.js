...
function getUserLocation() {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        // Se o usuário negar acesso, centralizar na PUC Minas
        const pucMinasLocation = { lat: -19.9227916, lng: -43.9924877 }; // Localização da PUC Minas
        map.setCenter(pucMinasLocation);
        addCentralMarker(pucMinasLocation); // Adiciona o marcador na PUC Minas
      }
    );
  } else {
...
// Adicionar marcador central (usuário ou PUC Minas)
...
function addLocationMarkers() {
  locations.forEach((location) => {
    ...
    const infoWindow = new google.maps.InfoWindow({
      content: `<h3>${location.name}</h3><p>Materiais: ${location.materials.join(
        ", "
      )}</p>`,
    });
...
function getUserLocation() {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        // Se o usuário negar acesso, centralizar no local padrão
        map.setCenter(defaultLocation);
        addCentralMarker(defaultLocation);
      }
    );
